...
const Result = ({data,diet,health,cuisineType,mealType,setDiet,fetchData,setHealth,setCuisineType,setmealType}) => {
   console.log(data);
  ...
  return (
    ...
    <FormControl sx={{ m: 1, minWidth: 120 }}>
      ...
      <FormControl sx={{ m: 1, minWidth: 120 }}>
        <InputLabel id="demo-simple-select-helper-label">Health</InputLabel>
        <Select
          labelId="demo-simple-select-helper-label"
          id="demo-simple-select-helper"
          value={health}
...
<FormControl sx={{ m: 1, minWidth: 120 }}>
      ...
      <Button onClick={fetchData} variant="contained">Submit</Button></div>
        ...
        {data.hits?<div>
          {data.hits.map((items)=>{
            return(
              ...
              <div><img src={items.recipe.images.REGULAR.url} alt="" /></div>
              <p>{items.recipe.label}</p>
                <p>{Math.round(items.recipe.calories)}</p>
                <div>{items.recipe.digest.slice(0,12).map((digest)=>{
                  return(
                    <>
                      <p>{digest.label}</p>
